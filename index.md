...
<section class="flex flex-col justify-center items-center px-4 py-12 bg-white dark:bg-slate-900">
  ...
  <div class="max-w-prose text-base md:text-lg normal-case">
    <!-- Float the image and let the paragraph wrap -->
    <img
    ...
    <div class="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      {%- for post in posts_list -%}
        ...
        <article class="post-card grid grid-rows-[auto_auto_1fr_auto] gap-2 h-full{% if idx > 3 %} hidden{% endif %}">
          ...
          <h3 class="text-lg md:text-xl font-medium">{{ post.title | truncatewords: 10, '...' }}</h3>
          <p class="text-xs md:text-sm normal-case">{{ post.description | truncatewords: 20, '...' }}</p>
